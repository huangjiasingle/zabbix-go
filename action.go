...
func (api *API) ActionCreate(name, interval string) error {
	fmt.Println(fmt.Sprintf(actionPostTemplate, name, interval, name, api.Session, api.ID))
	payload := strings.NewReader(fmt.Sprintf(actionPostTemplate, name, interval, name, api.Session, api.ID))
